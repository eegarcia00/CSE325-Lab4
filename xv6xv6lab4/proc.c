...
thread_create(void (*fn) (void*), void *stack, void *arg)
{
	...
	*np->tf = *parent_proc->tf;
	np->tf->eax = 0; //so child thread returns 0
	np->tf->eip = (uint)fn;
	//np->tf->eip = (uint)fn;
	//ustack[0] = 0xffffffff;
//	ustack[1] = (uint)arg;
...
//	copyout(np->pgdir, PGSIZE, ustack, PGSIZE);
	//np->tf->esp = ustack;
	////NEED TO FIX
	np->isthread = 1;
	...
	return pid;
	//copying necessary things from parent process
	//thread has own stack and program counter etc. but shares some resources
...
thread_join(void)
{
	...
	for(;;) {
		...
		for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
			if (p->parent != curproc)
				continue;
			if (p->isthread != 1) //if process identifies as a thread
				continue;
			havekids = 1;
			if (p->state == ZOMBIE) {
				pid = p->pid;
